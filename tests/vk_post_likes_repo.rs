...
use find_w::vk_post_likes::repo::{self, NewVkPostLike, VkPostLikeKey};
use sqlx::PgPool;
...
async fn vk_post_likes_upsert_inserts_and_updates_in_batch(pool: PgPool) {
    ...
    seed_post(&pool, user_id, 10, 1000, 1, 1_700_010_001).await;
    seed_post(&pool, user_id, 10, 1000, 2, 1_700_010_002).await;
    seed_post(&pool, user_id, 10, 1000, 3, 1_700_010_003).await;
...
async fn vk_post_likes_delete_is_batch_and_scoped_to_user(pool: PgPool) {
    ...
    seed_post(&pool, user_one, 20, 2000, 11, 1_700_010_011).await;
    seed_post(&pool, user_one, 20, 2000, 12, 1_700_010_012).await;
    seed_post(&pool, user_two, 20, 2000, 11, 1_700_010_111).await;
...
async fn vk_post_likes_cascade_from_vk_posts_and_vk_users(pool: PgPool) {
    ...
    seed_post(&pool, user_id, 30, 3000, 21, 1_700_010_021).await;
    seed_post(&pool, user_id, 30, 3001, 22, 1_700_010_022).await;
